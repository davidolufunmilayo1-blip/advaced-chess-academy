...
const original = { render, commit, choose, reset, save, tick, hint: $('#hint').onclick, undo: $('#undo').onclick, resign: $('#confirm-resign').onclick };
...
function switchView(view) {
  if (!['play','online','puzzles','learn','review','history','tournaments','train','tools','glossary','mates','openings','endgames','rush','coaches','profile','plan','clinic','lab'].includes(view)) return;
  activeView = view;
...
function drawStudyBoard(target, position, selectedSquare, onSquare, reverse=false, highlighted=[]) {
  const board = $(target); board.innerHTML = '';
  ...
  for(let pos=0;pos<64;pos++) {
    ...
    el.onclick=()=>onSquare?.(i); el.onkeydown=e=>{const delta={ArrowLeft:-1,ArrowRight:1,ArrowUp:-8,ArrowDown:8}[e.key];if(delta){e.preventDefault();board.children[Math.max(0,Math.min(63,pos+delta))].focus()}};
    board.append(el);
  }
}
...
$('#puzzle-filters').insertAdjacentHTML('beforebegin','<div class="feature-toolbar" id="puzzle-options"><label>Puzzle collection<select id="puzzle-source"><option value="endless">Endless puzzles</option><option value="library">Guided collection</option><option value="expert">Expert · mate in two</option></select></label><label>Difficulty<select id="puzzle-difficulty"><option value="easiest">Easiest (-600)</option><option value="easy" selected>Easier (-300)</option><option value="medium">Normal</option><option value="hard">Harder (+300)</option><option value="expert">Hardest (+600)</option></select></label></div><p id="endless-description">Unlimited generated mate-in-one practice. Difficulty describes board complexity, not a rating. Positions may eventually repeat.</p><p id="endless-score" role="status"></p>');
...
function startExpertPuzzle(index=expertIndex){++puzzleGeneration;expertIndex=index;$('#puzzle-source').value='expert';$('#puzzle-difficulty').value='expert';puzzleOptions();showPuzzle({...TRAINING.expert[index],level:'Hardest (+600)'},`Expert ${index+1} / ${TRAINING.expert.length}`);trainingSession.mistakes=0;trainingFeedback('No legal-move markers. Find the first move of a forced mate in two.')}
function chooseExpert(i){
 ...
 const correct=t.step===0?!!t.item.plans[uci]:core.check(next,next.turn)&&!core.allMoves(next).length;
 if(!correct){t.mistakes++;if(t.mistakes>=3){t.done=true;t.revealed=true;trainingFeedback('Three mistakes. Attempt finished—retry this challenge or reveal the line.','incorrect')}else trainingFeedback(`That move does not force the required mate. ${3-t.mistakes} mistakes remaining.`,'incorrect');rememberProgress();return renderTraining()}
 t.state=next;t.step++;if(t.step===3)return finishTraining();
 t.busy=true;trainingFeedback('Your opponent is defending…');renderTraining();
 const replies=Object.keys(t.item.plans[uci]);const reply=replies[Math.floor(Math.random()*replies.length)];
 trainingTimer=setTimeout(()=>{if(trainingSession!==t)return;t.state=core.apply(t.state,core.moveFromUCI(t.state,reply));t.step=2;t.busy=false;trainingFeedback('Find the checkmate.');renderTraining()},650);
}
...
$('#puzzle-difficulty').onchange=()=>{if($('#puzzle-difficulty').value==='expert')return startExpertPuzzle();lastGeneratedDifficulty=$('#puzzle-difficulty').value;$('#puzzle-source').value='endless';puzzleOptions();nextGeneratedPuzzle()};puzzleOptions();
...
$('#next-puzzle').onclick=()=>{if($('#puzzle-source').value==='expert')return startExpertPuzzle((expertIndex+1)%TRAINING.expert.length);if($('#puzzle-source').value==='endless')return nextGeneratedPuzzle();for(let n=1;n<=TRAINING.puzzles.length;n++){const i=(puzzleIndex+n)%TRAINING.puzzles.length;if(puzzleFilter==='All'||TRAINING.puzzles[i].theme===puzzleFilter)return startPuzzle(i)}};
$('#puzzle-retry').onclick=()=>{if(trainingSession?.item.expert)return startExpertPuzzle();if(trainingSession?.item.generated)showPuzzle(trainingSession.item,$('#puzzle-position').textContent);else startPuzzle(puzzleIndex)};
...
function receiveOnline(data){if(!online||data.id!==online.id||data.version<online.snapshot.version)return;const first=config.opponent!=='online',changed=data.version!==online.snapshot.version;online.snapshot=data;online.receivedAt=performance.now();online.error=null;state=data.state;past=data.past;notation=data.notation;remaining={...data.remaining};finished=data.result;started=data.started;config={opponent:'online',human:data.side,time:data.time,difficulty:2};if(changed){selected=null;pending=null;if($('#promotion').open)$('#promotion').close()}if(first||changed)render();else{drawClocks();renderOnline()}}
...
function renderOnline(){const box=$('#online-summary');box.hidden=!online;$('#online-chat').hidden=!online;for(const id of ['undo','hint','new-game'])$('#'+id).disabled=!!online||(id==='undo'?!past.length:id==='hint'?!!finished:false);$('.settings').querySelectorAll('select').forEach(s=>s.disabled=!!online);if(!online)return;const data=online.snapshot,opponent=data.players[core.other(online.side)],status=online.error||(data.waiting?(data.queue?'Looking for another player…':'Waiting for your friend to join…'):data.result?data.result.title:(state.turn===online.side?'Your move':'Opponent’s move')+(opponent?.connected?'':' · opponent reconnecting'));
box.innerHTML=`<h3><span class="spinner"></span>${data.waiting?'Your board is ready':'Online game'}</h3><div class="online-status" role="status">${escapeHTML(status)}</div><div class="muted">You play ${online.side==='w'?'White':'Black'} · Code ${data.id} · ${data.time/60} min</div><div class="inline-controls">${data.waiting?'<button class="secondary" id="copy-invite">Copy invitation</button>':'<button class="secondary" id="offer-draw">'+(data.drawOffer===core.other(online.side)?'Accept draw':data.drawOffer===online.side?'Draw offered':'Offer draw')+'</button>'}<button class="secondary" id="leave-online">${data.waiting?'Cancel search / room':'Leave board'}</button></div>${data.drawOffer===core.other(online.side)?'<button class="tiny-link" id="decline-draw">Decline draw offer</button>':''}`;
...
$('.mode').textContent='Online · '+data.time/60+' min';$('#status-detail').textContent=status;
...
(()=>{
  ...
  board.addEventListener('keydown',e=>{if(e.key==='Escape'){if(arrows.length)remember();clear();help.textContent='Drawings cleared.'}});
