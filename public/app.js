...
function switchView(view) {
  if (!['play','online','puzzles','learn','review','history','tournaments','train','tools','glossary','mates','openings','endgames','rush','coaches','profile','plan','clinic'].includes(view)) return;
  activeView = view;
...
$('#puzzle-filters').insertAdjacentHTML('beforebegin','<div class="feature-toolbar" id="puzzle-options"><label>Puzzle collection<select id="puzzle-source"><option value="endless">Endless puzzles</option><option value="library">Guided collection</option><option value="expert">Expert · mate in two</option></select></label><label>Difficulty<select id="puzzle-difficulty"><option value="easiest">Easiest</option><option value="easy" selected>Easier</option><option value="medium">Normal / Medium</option><option value="hard">Harder</option><option value="expert">Hardest</option></select></label></div><p id="endless-description">Unlimited generated mate-in-one practice. Difficulty describes board complexity, not a rating. Positions may eventually repeat.</p><p id="endless-score" role="status"></p>');
...
function startExpertPuzzle(index=expertIndex){++puzzleGeneration;expertIndex=index;$('#puzzle-source').value='expert';$('#puzzle-difficulty').value='expert';puzzleOptions();showPuzzle({...TRAINING.expert[index],level:'Hardest'},`Expert ${index+1} / ${TRAINING.expert.length}`);trainingSession.mistakes=0;trainingFeedback('No legal-move markers. Find the first move of a forced mate in two.')}
